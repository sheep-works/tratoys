...
const domain = "https://transheep.biz";
const domainAndSubdir = "https://transheep.biz/tratoys"
...
export default async function genSitemap(nitro: Nitro) {
    ...
    writeFileSync(path.join(publicDir, 'sitemap.xml'), data.toString().replaceAll(domain, domainAndSubdir));
}